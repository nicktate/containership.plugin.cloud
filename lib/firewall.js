            ...
            this.rule_queue = async.queue((task, callback) => {
                ...
                if(task.chain === this.options.chain.input) {
                    return async.parallel({
                        ...
                        const rules = _.union(this.get_initial_rules().input, results.host_rules.input, results.cloud_rules);
                    ...
                    async.series([
                        (callback) => {
                            return this.get_host_rules((err, host_rules) => {
                                ...
                                rules = _.union(this.get_initial_rules().forward, host_rules.forward);
                                return callback();
                            });
                        },
                        (callback) => {
    ...
    get_cloud_forward_rules(callback) {
        ...
        return async.parallel({
            ...
            results.firewalls = _.map(results.firewalls, (rule) => {
                if(rule.type === 'application' && endpoints[rule.application] && this.host.getOperatingMode() === 'follower') {
                    return _.map(endpoints[rule.application], (endpoint) => {
                        const flannel_rule = {
                            ...
                            destination_interface: 'flannel.1',
                            policy: 'ACCEPT',
                            protocol: rule.protocol
                        };
                        ...
                        return [flannel_rule, docker0_rule, private_flannel_rule, private_docker0_rule];
                    });
                }
            });
